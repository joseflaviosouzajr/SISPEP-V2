...
public function cadprontmed(){
   $cd_usuario_med=$_SESSION['cd_usuario'];
   $con = Conexao::getInstance();
...
if ($reg->finalizado != 'S') 
{
    ...
    echo "<td class='text-center'>  <a href='#'> <i data-pag='view/prontuario/prontuario_medico.php?atd=".$reg->cd_atendimento."&cd_ficha=".$reg->cd_ficha."' class='fas fa-pen'></i></a></td>";
...
public function editfichamed($ficha){

   $con = Conexao::getInstance();
   $query = "
   update ficha_med 
   set  ds_conduta = :ds_conduta, ds_queixa = :ds_queixa, motivo_alta = :motivo_alta, aceite_protocolo = :aceite
   where cd_ficha = :cd_ficha"  ;

   $stmt=$con->prepare( $query);
   
   $stmt->bindParam(':ds_conduta',$this->conduta);
   $stmt->bindParam(':ds_queixa',$this->queixa);   
   $stmt->bindParam(':motivo_alta',$this->motivo_alta);
   $stmt->bindParam(':aceite',$this->aceite_prot);
   $stmt->bindParam(':cd_ficha', $ficha);
   // var_dump($query);
